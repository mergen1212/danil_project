...
<script lang="ts">
	...
	let user: UserAuth = $state<UserAuth>({
		email: '',
		password: '',
		confirmPassword: ''
	});
	// Обработчик отправки формы
	let valid: boolean = $derived(user.password == user.confirmPassword && user.password.length >= 8);
	const handleSubmit = (event: Event) => {
		event.preventDefault();
		if (!valid) {
			alert('Пароли не совпадают и или короткий пароль');
			return;
		}
	...
	// Инициализация Telegram Passport после загрузки страницы
	$effect(() => {
		if (typeof Telegram !== 'undefined') {
			Telegram.Passport.createAuthButton('telegram_passport_auth', {
				bot_id: 8116276430, // YOUR BOT ID
				scope: {
					data: ['email'],
					v: 1
				},
				public_key: `-----BEGIN PUBLIC KEY-----

VwIDAQAB
-----END PUBLIC KEY-----`,
				nonce: 'thisisatest', // UNIQUE VALUE FOR THIS REQUEST
				callback_url: 'https://example.org' // MUST BE HTTPS
			});
		} else {
...
<div
	class="flex min-h-screen items-center justify-center bg-gray-100 transition-colors duration-300 dark:bg-gray-900"
	data-theme={total}
>
	<div class="w-full max-w-md rounded-lg bg-white p-8 shadow-lg dark:bg-gray-800">
		...
		<button
			...
			class="absolute top-4 right-4 rounded-lg bg-gray-200 p-2 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600"
			data-theme={total}
		>
			{#if darkMode}
		...
		<h2 class="mb-6 text-center text-2xl font-bold text-gray-800 dark:text-white">Регистрация</h2>
		<form
			onsubmit={(event) => {
				handleSubmit(event);
			}}
		>
			...
			<button
				type="submit"
				class="w-full rounded-md bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none dark:bg-indigo-500 dark:hover:bg-indigo-600"
			>
				Зарегистрироваться
			</button>
		</form>

		<!-- Telegram Passport Auth Button -->
		<div class="mt-6">
			<h3 class="text-center text-sm font-medium text-gray-700 dark:text-gray-300">
				Или войдите через Telegram Passport
			</h3>
			<div id="telegram_passport_auth" class="mt-6 text-center"></div>
		</div>
